...
var GLOBAL_PROBABILITY_OF_DEATH : float = 0.1
var GLOBAL_PROBABILITY_OF_REPLICATION : float = 0.5
var GLOBAL_PROBABILITY_OF_MUTATION : float = 0.5

var BIOME_SELECTIVITY_TEMPARATE : float = 2.0
var BIOME_SELECTIVITY_HOT : float = 2.0
var BIOME_SELECTIVITY_COLD : float = 2.0

var MUTATION_INTENSITY : float = 0.2
	...
	for i in range(WIDTH):
		...
		for j in range(HEIGHT):
			if rand_range(0.0, 1.0) < 0.1:
				matrix[i].append({"type":"tree", "species":Vector3(1.0, 0.5, 0.5).normalized(), "biome":"hot"})
			else:
				matrix[i].append({"type":"land", "biome":"hot"})
	return matrix

func create_init_state() -> Array:
	var matrix : Array = []
	for i in range(WIDTH):
		matrix.append([])  
		for j in range(HEIGHT):
